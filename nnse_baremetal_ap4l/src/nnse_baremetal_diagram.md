...
flowchart LR
  ...
  subgraph ACQ[PDM Acquisition]
    PDM_MIC["PDM Microphones"] -->|PDM bitstream| PDM_DMA["DMA / PDM IRQ"]
    PDM_DMA -->|writes| PDM_RING["PDM Ring Buffers"]
  end

  %% PDM -> PCM conversion
  PDM_RING -->|decimate + FIR| PCM_CONV["PDM to PCM Conversion"]
  ...
  I2S_DMA -->|I2S frames| DAC["External DAC (I2S)"]
  ...
  subgraph NOTES[ ]
    ...
    note2(["Buffers: double/triple buffering or ring buffers decouple IRQs and inference latency"]) 
  end
